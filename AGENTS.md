...
- Keep the exported surface minimal and purpose-built for whatever call shape is currently being verified — this is not a general-purpose FFI library, mirroring `mbt-wasmkit-ios/guest`'s convention.

### Kotlin (`shared/`, `iosApp/`)
...
Scope is optional; use the area name when the change targets a specific part of the tree (e.g., `guest`, `shared`, `iosApp`). Omit for project-wide changes.
